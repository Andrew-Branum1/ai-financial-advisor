...
class UniversalPortfolioEnv(gym.Env):
    ...
    def __init__(self, df: pd.DataFrame,
        ...
        self.action_space = spaces.Box(low=0, high=1, shape=(self.top_k_stocks,), dtype=np.float32)
        ...
        self.observation_space = spaces.Dict({
            'features': spaces.Box(low=-np.inf, high=np.inf, shape=(len(self.all_tickers), self.window_size, self.num_features), dtype=np.float32),
            'mask': spaces.Box(low=0, high=1, shape=(len(self.all_tickers),), dtype=np.int8) # Mask for stocks active on a given day
        })
    ...
    def step(self, action):
        ...
        if not np.all(np.isfinite(allocation_weights)):
            allocation_weights = np.nan_to_num(allocation_weights, nan=0.0, posinf=1.0, neginf=0.0)
        allocation_weights = np.clip(allocation_weights, 0, 1e6) # Clip to a large but finite number
        ...
        cash = self.portfolio_value if not self.portfolio_composition else liquidated_value
        ...
        if not np.isfinite(cash) or cash <= 0:
            ...
            terminated = True
            truncated = True
            return self._get_observation(), -1e6, terminated, truncated, self._get_info()
        ...
        self.portfolio_value = next_day_value if np.isfinite(next_day_value) else self.portfolio_value
    ...
    def _get_observation(self):
        ...
        features_array = np.zeros(self.observation_space['features'].shape, dtype=np.float32)
        ...
        for i, ticker in enumerate(self.all_tickers):
            if ticker in active_tickers_today:
                ...
                if len(ticker_df) == self.window_size:
                    ...
                    normalized_df = (ticker_df - ticker_df.mean()) / (ticker_df.std() + 1e-8)
...
# ... rest of the class ...
    ...
    def _calculate_reward(self):
        ...
        portfolio_values = self.initial_balance * (1 + np.array(self.daily_returns_history)).cumprod()
    ...
    def _get_info(self):
        return {"portfolio_value": self.portfolio_value, "date": self.trade_dates[self.current_step_index]}
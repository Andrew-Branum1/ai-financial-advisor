#!/usr/bin/env python3
"""
Final training script for short-term portfolio models.
...
def load_best_hyperparameters(strategy="short_term"):
    """
...
def create_env(df, env_params, is_training=True):
    ...
    Returns:
        PortfolioEnvShortTerm: Configured environment
    """
...
def train_short_term_final_model():
    """
    Train the final short-term portfolio model using optimized hyperparameters.
    """
    logger.info("Starting final short-term portfolio model training...")

    # Load best hyperparameters
    try:
        best_params = load_best_hyperparameters("short_term")
        ...
        env_params = {
            ...
            "max_concentration_per_asset": 0.3,
    ...
    model_dir = f"models/short_term_final_{timestamp}"
    os.makedirs(model_dir, exist_ok=True)

    # Setup callbacks
    checkpoint_callback = CheckpointCallback(
        save_freq=10000, save_path=model_dir, name_prefix="short_term_model"
    )
    ...
    model = PPO(
        ...
        train_env,
        verbose=1,
        tensorboard_log=f"logs/short_term_final_{timestamp}",
        device="cpu",
        **ppo_params,
    )
...
if __name__ == "__main__":
    try:
        model, model_dir = train_short_term_final_model()

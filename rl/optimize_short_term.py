#!/usr/bin/env python3
"""
Optuna hyperparameter optimization for short-term portfolio models.
...
def objective(trial):
    """
    Optuna objective function for short-term model hyperparameter optimization.
    ...
    # Use shorter periods for faster optimization
    ...
    env_params = {
        ...
        "rolling_volatility_window": trial.suggest_int(
            "rolling_volatility_window", 10, 30
        ),
        "momentum_weight": trial.suggest_float("momentum_weight", 0.1, 0.5),
        "mean_reversion_weight": trial.suggest_float("mean_reversion_weight", 0.1, 0.5),
        "volatility_target": trial.suggest_float("volatility_target", 0.10, 0.25),
        "turnover_penalty_weight": trial.suggest_float(
            "turnover_penalty_weight", 0.001, 0.01
        ),
        "max_concentration_per_asset": trial.suggest_float(
            "max_concentration_per_asset", 0.2, 0.6
        ),
        "min_holding_period": trial.suggest_int("min_holding_period", 1, 10),
    }
    ...
    try:
        ...
        # Create environments
        train_env = PortfolioEnvShortTerm(
            df=df_train_subset,
        ...
        model.learn(total_timesteps=50000)  # Reduced for faster optimization
...
def optimize_short_term_hyperparameters():
    """
    Run hyperparameter optimization for short-term models.
    """
    print("Starting hyperparameter optimization for short-term models...")
    ...
    # Run optimization
    try:
        study.optimize(objective, n_trials=30, timeout=7200)  # 2 hours timeout
    except KeyboardInterrupt:
        print("Optimization interrupted by user.")

    # Save results
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    results_dir = f"optimization_results/short_term_{timestamp}"
...
if __name__ == "__main__":
    try:
        best_params = optimize_short_term_hyperparameters()

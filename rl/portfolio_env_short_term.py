...
class PortfolioEnvShortTerm(gym.Env):
    ...
    def __init__(self, df: pd.DataFrame, feature_columns_ordered: list, **kwargs):
        ...
        self.portfolio_values_history = collections.deque(maxlen=max(self.momentum_lookback, self.mean_reversion_lookback))
        self.reset()
    ...
    def _calculate_enhanced_reward(self, daily_return: float, turnover: float) -> float:
        ...
        if len(self.portfolio_values_history) >= self.momentum_lookback:
            recent_values = list(self.portfolio_values_history)[-self.momentum_lookback:]
        ...
        if len(self.portfolio_values_history) >= self.mean_reversion_lookback:
            long_term_values = list(self.portfolio_values_history)[-self.mean_reversion_lookback:]
            if len(long_term_values) >= 2:
                ...
                mean_reversion_penalty = -self.mean_reversion_weight * abs(long_term_return)
    ...
    def _get_observation(self):
        ...
        obs_mean = np.mean(obs_window, axis=0)
        obs_std = np.std(obs_window, axis=0) + 1e-9
        return (obs_window - obs_mean) / obs_std

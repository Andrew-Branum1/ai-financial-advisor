...
def objective(trial):
    ...
    train_env_trial = PortfolioEnvAlpha(df=df_optuna_train, feature_columns_ordered=FEATURES_TO_USE_IN_MODEL, volatility_penalty_weight=env_volatility_penalty, loss_aversion_factor=env_loss_aversion, rolling_volatility_window=env_rolling_vol_window, turnover_penalty_weight=env_turnover_penalty, max_concentration_per_asset=env_max_concentration)
    validation_env_trial = PortfolioEnvAlpha(df=df_optuna_validation, feature_columns_ordered=FEATURES_TO_USE_IN_MODEL, volatility_penalty_weight=0.0, loss_aversion_factor=1.0)

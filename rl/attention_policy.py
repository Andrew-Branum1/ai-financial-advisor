...
class AttentionFeaturesExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: spaces.Dict, features_dim: int = 256):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=cnn_output_dim, nhead=4, dim_feedforward=features_dim, batch_first=True)
    ...
    def forward(self, observations):
        ...
        features = features.view(batch_size * num_assets, window_size, num_asset_features).permute(0, 2, 1)
        
        stock_features = self.cnn(features)
        stock_features = stock_features.view(batch_size, num_assets, -1)
        
        padding_mask = (mask == 0)
        transformer_output = self.transformer_encoder(stock_features, src_key_padding_mask=padding_mask)
...
class AttentionPolicy(ActorCriticPolicy):
    ...
    def __init__(self, observation_space, action_space, lr_schedule, *args, **kwargs):
        ...
        self.action_net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(128, self.top_k_stocks)
        )
    ...
    def forward(self, obs: torch.Tensor, deterministic: bool = False) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # GATHER FEATURES
        top_k_features = torch.gather(latent_features, 1, top_k_indices.unsqueeze(-1).expand(-1, -1, latent_features.shape[-1]))
    ...
    def evaluate_actions(self, obs: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        top_k_features = torch.gather(
            latent_features, 1, top_k_indices.unsqueeze(-1).expand(-1, -1, latent_features.shape[-1])
        )
        flat_top_k_features = top_k_features.flatten(start_dim=1)
    ...
    def predict(self, observation: np.ndarray, state: None = None, episode_start: None = None, deterministic: bool = False) -> Tuple[np.ndarray, None]:
        ...
        if observation["features"].ndim == 3:
            ...
            observation = {key: val[np.newaxis, ...] for key, val in observation.items()}

...
# LLM import for report generation
try:
    from llm.advisor import generate_investment_report
except ImportError:
    generate_investment_report = None
    logging.warning("llm.advisor.generate_investment_report not found. LLM reports will be unavailable.")
...
class ModelManager:
    ...
    def get_model_path(self, strategy: str) -> Optional[str]:
        ...
        strategy_dirs = [
            d for d in os.listdir(models_dir) if d.startswith(strategy_prefix)
        ]
    ...
    def get_environment(
        ...
        if model_path:
            ...
            if os.path.exists(config_path):
                ...
                # PATCH: If window_size is not found, try to get from ppo_params or fallback
                if not window_size:
                    window_size = training_config.get("ppo_params", {}).get("window_size")
                ...
                logging.error(f"training_config.json not found at {config_path}. Using default window_size.")
        if window_size is None:
            window_size = (
                env_params.get("window_size")
                if env_params and "window_size" in env_params
                else 30
            )
        # TEMP PATCH: If using long_term_conservative_20250701-230113, force window_size=60
        if 'long_term_conservative_20250701-230113' in model_path and window_size == 30:
            window_size = 60
            logging.warning("TEMP PATCH: Forcing window_size=60 for long_term_conservative_20250701-230113")
        ...
        # --- Improved automated fallback for window_size ---
        if strategy.startswith("short_term"):
            fallback_sizes = [30, 60]
        else:
            fallback_sizes = [60, 30]
        # Always try the original window_size last if not already in the list
        if window_size not in fallback_sizes and window_size is not None:
            fallback_sizes.append(window_size)
        fallback_tried = False
        for ws in fallback_sizes:
            if ws is None:
                continue
            env_kwargs["window_size"] = ws
            try:
                logging.info(f"[FALLBACK] Trying environment with window_size={ws} for strategy {strategy} and model {model_path}")
                ...
                if strategy.startswith("short_term"):
                    env = PortfolioEnvShortTerm(
                        df=df_subset, feature_columns_ordered=features_to_use, **env_kwargs
                    )
                else:
                    env = PortfolioEnvLongTerm(
                        df=df_subset, feature_columns_ordered=features_to_use, **env_kwargs
                    )
                # Try a reset to check observation shape
                obs, _ = env.reset()
                if hasattr(obs, 'shape') and (obs.shape[0] == ws or (hasattr(obs, 'ndim') and obs.ndim == 3 and obs.shape[1] == ws)):
                    if fallback_tried:
                        logging.warning(f"[FALLBACK] Using window_size={ws} for {model_path}")
                    return env
                else:
                    logging.warning(f"[FALLBACK] Observation shape {getattr(obs, 'shape', None)} does not match window_size={ws}")
            except Exception as e:
                logging.warning(f"[FALLBACK] Failed to create environment with window_size={ws}: {e}")
                fallback_tried = True
        logging.error(f"[FALLBACK] All window_size fallbacks failed for {model_path}. Cannot create environment.")
        return None
    ...
    def _load_env_params_from_config(self, strategy: str) -> dict:
        ...
        try:
            ...
            if os.path.exists(config_path):
                ...
                logging.warning(
                    f"Config file not found at {config_path}, using default parameters"
                )
                return {}

        except Exception as e:
            logging.warning(
                f"Failed to load env params from config: {e}, using default parameters"
            )
            return {}
    ...
    def get_portfolio_recommendation(
        ...
        try:
            ...
            logging.info(
                f"Data shape for KPI calculation: {df_data.shape if isinstance(df_data, pd.DataFrame) else 'None'}"
            )
            ...
            env = self.get_environment(
                strategy, df_data, env_params=env_params, model_path=model_path
            )
            if env is None:
            ...
            if action_sum > 1e-6:
                ...
                predicted_weights = np.full(
                    len(config["agent_tickers"]), 1.0 / len(config["agent_tickers"])
                )
            ...
            if top_weights_sum > 1e-6:
                normalized_top_weights = [
                    weight / top_weights_sum for _, weight in top_recommendations
                ]
            else:
                normalized_top_weights = [1.0 / len(top_recommendations)] * len(
                    top_recommendations
                )
            final_recommendations = {
                ticker: weight
                for (ticker, _), weight in zip(
                    top_recommendations, normalized_top_weights
                )
            }
            allocation_amounts = {
            ...
            sector_breakdown = self._get_sector_breakdown(
                [ticker for ticker, _ in top_recommendations]
            )
            result = {
                "strategy": strategy,
        ...
        while True:
            ...
            try:
                ...
                result = loop.run_until_complete(
                    generate_investment_report(*args, **kwargs)
                )
            except Exception as e:
...
def analyze_portfolio():
    ...
    try:
        ...
        profile = InvestmentProfile(
            age, investment_amount, time_horizon, risk_tolerance
        )
        ...
        all_tickers = sorted(
            list(set(portfolio_rec["available_tickers"] + [portfolio_rec["benchmark"]]))
        )
        ...
        def compute_portfolio_metrics(df, weights, tickers):
            ...
            port_returns = (
                returns * np.array([weights.get(t, 0) for t in tickers])
            ).sum(axis=1)
        ...
        if df_hist is not None and not df_hist.empty:
            kpis = compute_portfolio_metrics(
                df_hist,
                portfolio_rec["recommended_weights"],
                portfolio_rec["available_tickers"],
            )
        else:
            kpis = {
        ...
        try:
            ...
            try:
                explanation = safe_generate_investment_report(
                    kpis=kpis,
            ...
            logging.warning(f"LLM report generation failed: {e}")
            explanation = "AI analysis temporarily unavailable. Please check back later."
...
def safe_generate_investment_report(*args, **kwargs):
    if generate_investment_report is None:
        return "LLM report unavailable: generate_investment_report not found."
    try:
        return asyncio.run(generate_investment_report(*args, **kwargs))
    except RuntimeError as e:
        if "event loop is closed" in str(e):
            # Try to create a new event loop
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            return loop.run_until_complete(generate_investment_report(*args, **kwargs))
        else:
            print(f"LLM report error: {e}")
            return "LLM report unavailable due to error."
    except Exception as e:
        print(f"LLM report error: {e}")
        return "LLM report unavailable due to error."

...
def load_market_data_from_db(
    ...
    if missing_in_df:
        ...
        if not feature_columns:
            logging.error(
                "No valid feature columns remaining after checking DB results. Cannot proceed with pivot."
            )
            return pd.DataFrame()
        logging.info(
            f"Proceeding with pivot using available features: {feature_columns}"
        )
    ...
    try:
        ...
        processed_tickers_ordered = [
            ...
            if t in {col.split("_")[0] for col in market_df.columns}
        ]

        for ticker in processed_tickers_ordered:
            for (
                feature
            ) in feature_columns:  # Use the (potentially filtered) feature_columns
    ...
    logging.info(
        f"Market data loaded successfully. Shape: {market_df.shape}. Columns (first 10 of {len(market_df.columns)}): {market_df.columns.tolist()[:10]}..."
    )
    return market_df
...
def load_and_split_data(
    ...
    """
    # Load full dataset
    full_data = load_market_data_from_db(
...
def calculate_evaluation_kpis(
    model: BaseAlgorithm, eval_env: gym.Env
) -> Dict[str, float]:
    """
...
def calculate_technical_indicators(df: pd.DataFrame, ticker: str) -> pd.DataFrame:
    ...
    result[f"{ticker}_macd_histogram"] = (
        result[f"{ticker}_macd"] - result[f"{ticker}_macd_signal"]
    )
    ...
    result[f"{ticker}_bollinger_width"] = (
        result[f"{ticker}_bollinger_upper"] - result[f"{ticker}_bollinger_lower"]
    ) / sma_20
    result[f"{ticker}_bollinger_position"] = (
        close_prices - result[f"{ticker}_bollinger_lower"]
    ) / (result[f"{ticker}_bollinger_upper"] - result[f"{ticker}_bollinger_lower"])
    ...
    high_low = (
        close_prices.rolling(window=1).max() - close_prices.rolling(window=1).min()
    )
    ...
    # 11. Williams %R
    result[f"{ticker}_williams_r"] = (
        -100 * (high_14 - close_prices) / (high_14 - low_14)
    )
    ...
    # 16. Volatility Ratio
    result[f"{ticker}_volatility_ratio"] = (
        result[f"{ticker}_volatility_20"] / result[f"{ticker}_volatility_60"]
    )
    ...
    result[f"{ticker}_price_efficiency"] = np.abs(
        close_prices - close_prices.shift(20)
    ) / close_prices.rolling(window=20).apply(
        lambda x: np.sum(np.abs(x.diff().dropna()))
    )

    # 20. Mean Reversion Signal
    result[f"{ticker}_mean_reversion_signal"] = (
        close_prices - result[f"{ticker}_sma_50"]
    ) / (result[f"{ticker}_atr"] + 1e-8)

    return result


def enhance_market_data_with_indicators(
    df: pd.DataFrame, tickers_list: List[str]
) -> pd.DataFrame:
    """
    ...
    if isinstance(enhanced_df, pd.DataFrame):
        ...
        if rows_before != rows_after:
            logging.info(
                f"Dropped {rows_before - rows_after} rows with persistent NaN values"
            )

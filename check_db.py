...
def check_database():
    ...
    try:
        ...
        counts = pd.read_sql_query(
            "SELECT ticker, COUNT(*) as count FROM price_data GROUP BY ticker", conn
        )
        print(counts)
        ...
        date_range = pd.read_sql_query(
            "SELECT MIN(date) as min_date, MAX(date) as max_date FROM price_data", conn
        )
        print(date_range)
        ...
        for ticker in target_tickers:
            count = pd.read_sql_query(
                f"SELECT COUNT(*) as count FROM price_data WHERE ticker = '{ticker}'",
                conn,
            )

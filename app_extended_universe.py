...
class ExtendedUniverseModelManager:
    ...
    def get_portfolio_recommendation(self, profile: InvestmentProfile) -> Any:
        ...
        try:
            ...
            tickers = get_ticker_universe(profile.universe_size, profile.selection_strategy)
            ...
            top_weights = [rec[2]/100 for rec in top_recommendations]  # Convert percentage to decimal
    ...
    try:
        ...
        if llm_advisor:
            try:
                ...
                top_stocks = [rec['ticker'] for rec in recommendation['top_recommendations']]
                allocations = [rec['percentage'] for rec in recommendation['top_recommendations']]
                
                # Create weights dictionary for LLM
                weights_dict = {ticker: allocation/100 for ticker, allocation in zip(top_stocks, allocations)}
...
if __name__ == '__main__':
    ...
    logger.info("Available selection strategies: diversified, momentum, value, balanced")
